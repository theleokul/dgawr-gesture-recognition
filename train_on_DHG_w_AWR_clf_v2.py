...
FM = FeatureModule()
# detector_coef_policy = lambda epoch: np.clip(100_000 - 99_000 * (epoch / 15), 1_000, 100_000)
detector_coef_policy = lambda _: 10_000
...
def init_model(data_cfg):
    ...
    model_detection.load_state_dict(
        torch.load('/home/l.kulikov/dev/DGAWR/exp_detection_only/DHS_ID-3_dp-0.2_lr-0.001_dc-0/epoch_147_acc_0.000234376.pth')['model_detection']
    )
...
def main(exp_name, args):
    ...
    model_fold = "/home/l.kulikov/dev/DGAWR/{}/DHS_ID-{}_dp-{}_lr-{}_dc-{}/".format(exp_name, args.test_subject_id,args.dp_rate, args.learning_rate, args.data_cfg)
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batched in pbar:
            ...
            loss = detector_coef_policy(epoch) * loss_detection + loss_clf
            ...
            pbar.set_postfix({
                'cur_detection_coef': detector_coef_policy(epoch),
                'loss_detection': detector_coef_policy(epoch) * train_loss_detection.item() / float(i + 1),
...
if __name__ == "__main__":
    ...
    for tsi in range(1, 21):
        args.test_subject_id = tsi
        exp_name = f'20220417_clf_only/tsi_{tsi}'
        main(exp_name, args)
    
    
# CLF only
# 0.8356, 0.7714, 0.9286, 0.8929, 0.8000, 0.8429, 0.8857, 0.9571, 0.8857, 0.9357, 0.9214, 0.8786, 0.8214, 0.8643, 0.8857, 0.9500, 0.8500, 0.9071, 0.9714, 0.9000
# mean = 0.8843

# Dynamic training w/ stable detection coefficient
# 0.8357, 0.7571, 0.8857, 0.9000, 0.6643, 0.7857, 0.8857, 0.9143, 0.7500, 0.8000, 0.9071, 0.8357, 0.8214, 0.8857, 0.8929, 0.8714, 0.8286, 0.8714, 0.9429, 0.8929
# mean = 0.8464

# Dynamic training w/ decreasing detection coefficient
# 0.8000, 0.7786, 0.8929, 0.8929, 0.6429, 0.7714, 0.8857, 0.9214, 0.7357, 0.8214, 0.8571, 0.7428, 0.8286, 0.9286, 0.8643, 0.7929, 0.8000, 0.8429, 0.9143, 0.9071
# mean = 0.8311
